...
CvScalar Slic::Get2D(Mat &img, int idx0, int idx1)
{
    cv::Vec3b cl = img.at<cv::Vec3b>(idx0,idx1);
    CvScalar colour;
    colour.val[0] = cl[0];colour.val[1] = cl[1];colour.val[2] = cl[2];
    return colour;
}
...
//            cv::Vec3b cl = image.at<cv::Vec3b>(nc.y,nc.x);
//            CvScalar colour;
//            colour.val[0] = cl[0];colour.val[1] = cl[1];colour.val[2] = cl[2];

            CvScalar colour = Get2D(image, nc.y, nc.x);
 ...
 */
CvPoint Slic::find_local_minimum(Mat &image, CvPoint center) {
    ...
    for (int i = center.x-1; i < center.x+2; i++) {
        for (int j = center.y-1; j < center.y+2; j++) {
            ...
            CvScalar c1 = Get2D(image, j+1, i);
            CvScalar c2 = Get2D(image, j, i+1);
            CvScalar c3 = Get2D(image, j, i);
...
//                        cv::Vec3b cl = image.at<cv::Vec3b>(l,k);
//                        CvScalar colour;
//                        colour.val[0] = cl[0];colour.val[1] = cl[1];colour.val[2] = cl[2];

                        CvScalar colour = Get2D(image, l, k);
        ...
        for (int j = 0; j < image.cols; j++) {
            for (int k = 0; k < image.rows; k++) {
                ...
                if (c_id != -1) {
                    CvScalar colour = Get2D(image, k, j);
 ...
 * Output: -
 */
void Slic::display_center_grid(Mat &image, CvScalar colour) {
    for (int i = 0; i < (int) centers.size(); i++) {
        cvCircle(image.data, cvPoint(centers[i][3], centers[i][4]), 2, colour, 2);
    }
}
 ...
 * Output: -
 */
void Slic::display_contours(Mat &image, CvScalar colour) {
    ...
    for (int i = 0; i < (int)contours.size(); i++) {
        //cvSet2D(image.data, contours[i].y, contours[i].x, colour);
        cv::Vec3b cl;
        cl[0] = colour.val[0];cl[1] = colour.val[1];cl[2] = colour.val[2];
        image.at<cv::Vec3b>(contours[i].y,contours[i].x) = cl;
    }
}
...
void Slic::colour_with_cluster_means(Mat &image) {
    ...
    for (int i = 0; i < image.cols; i++) {
        for (int j = 0; j < image.rows; j++) {
            ...
            CvScalar colour = Get2D(image, j, i);
    ...
    for (int i = 0; i < image.cols; i++) {
        for (int j = 0; j < image.rows; j++) {
            ...
            //cvSet2D(image.data, j, i, ncolour);
            cv::Vec3b cl;
            cl[0] = ncolour.val[0];cl[1] = ncolour.val[1];cl[2] = ncolour.val[2];
            image.at<cv::Vec3b>(j,i) = cl;
        }
    }
}
